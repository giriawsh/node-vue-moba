module.exports = app => {
    ...
    router.get('/news/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([
            ...
            {
                $addFields: { //添加字段
                    newsList: {$slice: ['$newsList', 2]}//只要其中的2条
                }
            }
        ])//聚合查询
        ...
        cats.unshift({
            ...
            }).limit(2).populate('categories').lean()
        })
        cats.map(cat => {
    ...
    router.get('/heroes/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([
            ...
            // {
            //     $addFields: { //添加字段
            //         newsList: {$slice: ['$heroList', 2]}//只要其中的2条
            //     }
            // }
        ])//聚合查询
        const subCats = cats.map(v => v._id);
    ...
    router.get('/articles/:id', async (req, res) => {
        const data = await Article.findById(req.params.id).lean();//变成纯粹的json对象
